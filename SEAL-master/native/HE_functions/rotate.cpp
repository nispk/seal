...
void rotate_mult(shared_ptr<SEALContext> context, Ciphertext input)
{
    ...
    cout << " temp value after adding max_input_plain " << intencoder.decode_int64(dec) << endl;
    ...
    for (int j = 0; j <= N; j++)
    {
        ...
        func_indices.push_back(intencoder.encode(j)); // = 0, 1, ..., 16
    ...
    for (size_t s = 0; s < n; s++)
    {
        ...
        for (int i = 0; i <= N; i++)
        {
            ...
            cout << intencoder.decode_int64(a1) << " * " << intencoder.decode_int64(a2) << "  =  " << intencoder.decode_int64(a3) << endl;
        }
